...
| `$listCatalog` `system.buckets.*`        | Time-series / bucket collections in each user database                 | Read-only `$listCatalog` per database on **data-bearing** `mongod` (skips `admin` / `local` / `config`; not mongos, not config-server members) |
